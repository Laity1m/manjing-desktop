...
export default function VoiceLibraryClient() {
  ...
  return <main className="portal-page voice-library-page">
    ...
    {groups.map((group) => {
      ...
      return <section key={group.id} className="voice-project-group"><header><div><span>{group.id === "global" ? "PUBLIC" : "PROJECT"}</span><h2>{group.name}</h2><p>{group.id === "global" ? "所有项目都可以按人物名引用；仅上传你有权使用或克隆的声音。" : "由该项目剧本分析生成的人物音色框架和已确认音色。"}</p></div><b>{items.length} 项</b></header>{items.length ? <div className="voice-card-grid">{items.map((voice) => <article key={voice.id} className={voice.assetState === "placeholder" ? "placeholder" : "ready"}><div className="voice-card-head"><i>声</i><span><b>{voice.identityKey || voice.name}</b><small>{voice.lookName || "标准音色"} · {voice.assetState === "placeholder" ? "等待选择来源" : voice.canonical ? "Canonical" : "待确认"}</small></span></div>{previews[voice.id] && <audio src={previews[voice.id]} controls preload="metadata" />}<input value={voice.identityKey || ""} placeholder="人物名字" onChange={(event) => setVoices((items) => items.map((item) => item.id === voice.id ? { ...item, identityKey: event.target.value } : item))} onBlur={(event) => void edit(voice, { identityKey: event.target.value, entityId: event.target.value })} /><textarea value={voice.referenceText || ""} placeholder="参考音频中的准确台词" onChange={(event) => setVoices((items) => items.map((item) => item.id === voice.id ? { ...item, referenceText: event.target.value } : item))} onBlur={(event) => void edit(voice, { referenceText: event.target.value })} /><p>{voice.semanticDescription}</p>{voice.assetState === "placeholder" ? <div className="voice-source-actions"><label>上传已有音色<input type="file" accept="audio/*,.mp3,.wav,.m4a,.ogg" onChange={(event) => { void bind(voice, event.target.files?.[0]); event.currentTarget.value = ""; }} /></label><button onClick={() => generate(voice)}>让 AI 生成参考音色</button></div> : <div className="voice-source-actions"><button className={voice.canonical ? "active" : ""} onClick={() => void confirmVoice(voice)}>{voice.canonical ? "✓ 已确认 Canonical" : "确认授权并设为 Canonical"}</button><button onClick={() => void edit(voice, { voiceConsent: "revoked", reusable: false, canonical: false })}>撤销授权</button></div>}<ConfirmButton onConfirm={() => remove(voice)} disabled={busy} ariaLabel={`删除音色 ${voice.name}`} confirmLabel="确认删除">删除</ConfirmButton></article>)}</div> : <div className="voice-project-empty">这个分组还没有音色或音色框架。</div>}</section>;
    })}
  </main>;
}
