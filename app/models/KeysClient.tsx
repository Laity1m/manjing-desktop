...
import { loadCustomModels, saveCustomModels, type CustomModel, type CustomModelAdapter, type CustomModelRole } from "../lib/custom-models";
...
export default function KeysClient() {
  ...
  useEffect(() => {
    const frame = requestAnimationFrame(() => setCustomModels(loadCustomModels()));
  ...
  function addCustomModel() {
    if (!draft.name.trim() || !draft.model.trim()) return setModelMessage("请填写模型名称和模型 ID");
    if (draft.adapter === "webhook" && !/^https?:\/\//i.test(draft.endpoint.trim())) return setModelMessage("Webhook 模型需要填写 http:// 或 https:// 接口地址");
    const model: CustomModel = { ...draft, id: `custom-${draft.role}-${Date.now().toString(36)}`, name: draft.name.trim(), model: draft.model.trim(), endpoint: draft.endpoint.trim(), apiKey: draft.apiKey.trim(), note: draft.note.trim() || "用户自定义模型" };
    const next = [model, ...customModels];
    setCustomModels(next);
    saveCustomModels(next);
    setDraft({ ...EMPTY_MODEL, role: draft.role });
    setModelMessage(`${model.name} 已加入 ${ROLE_LABELS[model.role]}，回到工作台即可选择。`);
  }
  ...
  return <main className="portal-page keys-page">
    ...
    <section id="custom" className="custom-model-builder">
      <div className="custom-model-heading"><div><span>MY MODEL LIBRARY</span><h2>每个 AI 岗位，都能添加自己的模型</h2><p>保存后会直接出现在 AI 工作台对应岗位的下拉列表中，密钥仍只保存在当前浏览器。</p></div><a href="/studio">返回 AI 工作台 →</a></div>
      <div className="custom-model-grid">
        <div className="custom-model-form">
          <div className="custom-role-tabs">{(Object.keys(ROLE_LABELS) as CustomModelRole[]).map((role) => <button key={role} className={draft.role === role ? "active" : ""} onClick={() => setDraft((value) => ({ ...value, role }))}>{ROLE_LABELS[role]}</button>)}</div>
          <div className="custom-form-fields">
            <label>显示名称<input value={draft.name} onChange={(event) => setDraft((value) => ({ ...value, name: event.target.value }))} placeholder={`例如：我的${ROLE_LABELS[draft.role]}`} /></label>
            <label>调用方式<select value={draft.adapter} onChange={(event) => setDraft((value) => ({ ...value, adapter: event.target.value as CustomModelAdapter }))}><option value="webhook">通用 Webhook / OpenAI 兼容</option><option value="pollinations">Pollinations</option>{draft.role === "video" && <option value="seedance">Seedance 方舟 API</option>}{["video", "editor"].includes(draft.role) && <option value="browser">浏览器本地处理</option>}</select></label>
            <label>模型 ID<input value={draft.model} onChange={(event) => setDraft((value) => ({ ...value, model: event.target.value }))} placeholder="模型名称、模型 ID 或 Endpoint ID" /></label>
            {draft.adapter === "webhook" && <label>接口地址<input value={draft.endpoint} onChange={(event) => setDraft((value) => ({ ...value, endpoint: event.target.value }))} placeholder="https://your-api.example/v1/generate" /></label>}
            {draft.adapter !== "browser" && <label>API Key / Bearer Token<input type="password" value={draft.apiKey} onChange={(event) => setDraft((value) => ({ ...value, apiKey: event.target.value }))} placeholder="可留空，稍后在岗位设置填写" /></label>}
            <label>备注<input value={draft.note} onChange={(event) => setDraft((value) => ({ ...value, note: event.target.value }))} placeholder="能力、费用或使用说明" /></label>
          </div>
          <button className="save-custom-model" onClick={addCustomModel}>＋ 保存到我的模型库</button>
          {modelMessage && <p className="custom-model-message">{modelMessage}</p>}
        </div>
        <div className="custom-model-list"><div><b>已添加模型</b><span>{customModels.length} 个</span></div>{customModels.length ? customModels.map((model) => <article key={model.id}><i>{ROLE_LABELS[model.role].slice(0, 1)}</i><div><b>{model.name}</b><span>{ROLE_LABELS[model.role]} · {model.adapter}</span><small>{model.model}{model.note ? ` · ${model.note}` : ""}</small></div><button onClick={() => removeCustomModel(model.id)}>删除</button></article>) : <div className="custom-model-empty"><i>＋</i><p>还没有自定义模型<br />从左侧选择岗位并填写接口</p></div>}</div>
      </div>
    </section>
  </main>;
}
