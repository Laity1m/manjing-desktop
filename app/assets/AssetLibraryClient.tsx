...
export default function AssetLibraryClient() {
  ...
  useEffect(() => {
    ...
    const trusted = new URLSearchParams(window.location.search).get("trusted") === "1";
    const blocked = (new URLSearchParams(window.location.search).get("blocked") || "").split(",").map((item) => item.trim()).filter(Boolean);
    queueMicrotask(() => {
      setSeriesProjects(projects);
      setTrustedMode(trusted);
      setBlockedTokens(blocked);
      if (trusted) { setFilter("character"); setCategory("character"); }
  ...
  const visible = useMemo(() => assets.filter((asset) => {
    ...
    return (!trustedMode || asset.category === "character") && (filter === "all" || asset.category === filter) && (!keyword || `${asset.name} ${asset.tags.join(" ")}`.toLocaleLowerCase("zh-CN").includes(keyword));
  }), [assets, filter, query, trustedMode]);

  function isBlockedAsset(asset: LibraryAsset) {
    if (!trustedMode || asset.category !== "character") return false;
    const identities = [asset.id, asset.identityKey || "", asset.entityId || "", asset.name].map(normalizedIdentity);
    return asset.tags.includes("Seedance真人拦截") || blockedTokens.some((token) => identities.includes(normalizedIdentity(token)));
  }
  ...
  function clearResolvedPortraitBlock(nextAssets: LibraryAsset[]) {
    try {
      ...
      if (!unresolved.length) {
        ...
        setMessage("所有被拦截人物均已绑定授权 Asset ID。返回工作台后可安全恢复视频生成。");
      }
    } catch { /* Invalid stale blocker metadata is ignored. */ }
  }
  ...
  async function editAsset(asset: LibraryAsset, patch: Parameters<typeof updateLibraryAsset>[1]) {
    ...
    clearResolvedPortraitBlock(nextAssets);
  }
  ...
  return <main className="portal-page asset-library-page">
    ...
    {trustedMode && <section className="trusted-portrait-center" aria-label="Seedance可信人物中心"><header><div><span>SEEDANCE TRUSTED PORTRAITS</span><h2>可信人物中心</h2><p>方舟已拦截可能含真人的参考图。企业高级/Premium 可直接在下方由漫镜创建授权任务、上传素材、轮询审核并回写 Asset ID；演员本人仍需完成一次方舟授权。</p></div><strong>{assets.filter((asset) => asset.category === "character" && asset.arkAssetId && asset.portraitAuthorizationStatus === "authorized").length}/{assets.filter((asset) => asset.category === "character").length} 已授权</strong></header><div className="trusted-portrait-steps"><span><b>1</b>一次填写企业信息</span><span><b>2</b>演员打开本人授权页</span><span><b>3</b>漫镜自动入库与回写</span><span><b>4</b>自动恢复中断的视频</span></div><div className="trusted-portrait-actions"><a href="https://console.volcengine.com/ark" target="_blank" rel="noreferrer">打开火山方舟控制台 ↗</a><a href="https://www.volcengine.com/docs/82379/2315856" target="_blank" rel="noreferrer">查看可信人像说明 ↗</a><button type="button" onClick={() => router.push("/studio")}>返回工作台</button></div></section>}
    <EnterpriseAssetPanel onAssetUpdated={() => void refresh()} />
    ...
    {projectVisible.length ? <section className="asset-library-grid">{projectVisible.map((asset) => <article key={asset.id} className={`${selected.includes(asset.id) ? "selected" : ""} ${asset.assetState === "placeholder" ? "placeholder" : ""} ${isBlockedAsset(asset) ? "portrait-blocked" : ""}`}>
