...
export default function AssetLibraryClient() {
  ...
  async function refresh() {
    const list = await listLibraryAssets({ allProjects: true });
    setAssets(list);
  ...
  return <main className="portal-page asset-library-page">
    ...
    <section className="asset-project-browser" aria-label="按项目查看资产"><header><div><span>PROJECT ASSETS</span><h2>项目资产</h2></div><p>进入项目后展示项目专属与全局资产；Agent 仍只调用当前绑定项目。</p></header><div className="asset-project-list"><button type="button" className={assetProjectId === "all" ? "active" : ""} onClick={() => { setAssetProjectId("all"); setExpandedProjectId(""); }}><i>∞</i><span><b>全部资产</b><small>{assets.length} 项</small></span><em>查看</em></button>{projectGroups.map((group) => <article key={group.id} className={`${assetProjectId === group.id ? "active" : ""} ${expandedProjectId === group.id ? "expanded" : ""}`}><button type="button" className="asset-project-main" onClick={() => setExpandedProjectId((value) => value === group.id ? "" : group.id)}><i>{group.name.slice(0, 1)}</i><span><b>{group.name}</b><small>{group.count} 项资产{group.episodes ? ` · ${group.episodes} 集` : ""}</small></span><em>{expandedProjectId === group.id ? "收起" : "展开"}</em></button>{expandedProjectId === group.id && <div className="asset-project-actions"><button type="button" onClick={() => { setAssetProjectId(group.id); setSelected([]); }}>进入项目资产</button><button type="button" onClick={() => setExpandedProjectId("")}>折叠</button></div>}</article>)}</div></section>
    ...
    {projectVisible.length ? <section className="asset-library-grid">{projectVisible.map((asset) => <article key={asset.id} className={selected.includes(asset.id) ? "selected" : ""}>
