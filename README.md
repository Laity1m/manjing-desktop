...
所有文本模型通道通过同一套 Skill 解析规则，按照当前项目、岗位、任务相关度和长度预算检索已启用 Skill。制作现场内置统一审计面板，逐次显示真正读取的项目记忆、Skill、`@Image` / `@Video` / `@Audio` 资产、被跳过的候选及原因和最终提交顺序；外部媒体 Webhook 同时收到编译后的岗位知识。
...
镜头属性检查器提供轻量顶视舞台：用户可拖动人物与相机图标，调节相机方向、视场角和俯仰近似。系统把舞台世界坐标数学投影为画面 `x / y / scale` 与容差边界，同一场景的后续镜头继承同一布局。开启位置冻结后，投影结果会进入 Seedance 最终提示词并禁止自动换边、居中和改变裁切。

该模式不引入 3D 引擎，也不会生成或提交首尾帧；极端俯视、仰视仍建议补充对应角度的 Canonical 参考资产。
