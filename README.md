...
- 每个镜头携带人物稳定世界锚点、左右顺序、景深、朝向、姿态、出入画方向、道具绑定和场景固定物位置。
- 原生视频不生成逐镜首帧图；人物、场景、道具与按需的上一镜尾帧都只作为 Seedance 全能参考，由模型直接建立开场状态。
- 摄像机改变时按同一世界位置重新投影，不机械冻结屏幕坐标；同场对话遵守 180 度轴线，避免人物无理由换边。
- 禁止人物和物体凭空生成、消失、复制、换手或瞬移；进入与离开必须经过画面边缘、门口或有依据的遮挡。
- 人物身份、演员数量、画风与空间连续性均设为硬性质量门槛，不合格候选不会自动进入成片或污染下一镜参考。

### 图片、视频、声音与剪辑

- 生图 Agent 可生成人物、造型、场景、道具和分镜图。
- 新增“半写实3D国漫”风格，区分于写意国漫电影感，并对人物资产与视频统一执行非真人化风格锁定。
...
- 原生视频流程完全跳过逐镜首帧生成与首帧质检；视频中间帧和尾帧只用于成片检查与可选连续性参考，不归档为分镜资产。
- 人物标准图采用“身份与服装解耦”：最左侧唯一身份大头照必须平视、严格正脸、双眼直视镜头；右侧正/侧/背三视图只承担体型与服装参考，其中正面和侧面五官区域使用背景色遮罩，避免生成多套互相冲突的脸。
- 生成人物出镜镜头时必须真实携带对应人物标准图；本地图片会转换为可传输参考图，OpenAI 兼容生图接口会自动切换到图片编辑模式。缺少任何出镜人物参考图时停止该镜头，不再无参考盲生成。
