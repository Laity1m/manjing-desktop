...
test("improves the free image and motion pipeline without presenting it as native animation", async () => {
  ...
  assert.match(page, /right 60%-65% is a vertically ordered three-view turnaround/);
  assert.match(page, /imageAspect: "16:9"/);
  assert.match(page, /Create one polished 16:9 production Canonical character card/);
  ...
  assert.match(page, /sheetVersion: 3 as const/);
  assert.match(page, /width: options\.imagePurpose === "character-card" \? "1792"/);
